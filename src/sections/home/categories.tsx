...
// Alias para el tipo de categoría literal
...
const Categories = forwardRef<CategoriesHandle>((_props, ref) => {
  ...
  // Creamos el array de categorías asegurando el tipo CategoryName[]
  const categories = Array.from(
    new Set(CATEGORY_PRODUCTS.map((p) => p.category))
  ) as CategoryName[]

  // Estado seleccionado con tipo CategoryName
  const [selected, setSelected] = useState<CategoryName>(() => categories[0])
  const [page, setPage] = useState<number>(1)
  const [modalVersion, setModalVersion] = useState<number>(0)

  // Si hay ?category= validamos y seleccionamos
  useLayoutEffect(() => {
  ...
  // Cargar wishlist y cart desde localStorage
  ...
  // Exponer scrollToCategory al padre
  ...
  // Registrar en CategoriesScrollContext
  useEffect(() => {
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-1/5">
            <h2 className="text-2xl font-bold mb-4 lg:mb-8 text-center lg:text-left">
              ¡EXPLORA NUESTRAS
              <br className="hidden lg:block" /> CATEGORÍAS DE PRODUCTOS!
            </h2>
